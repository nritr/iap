...
if(($request->getGet("term") && strlen($request->getGet("term")) > 0) || ($request->getGet("id") && is_numeric($request->getGet("id")))) {
    ...
    if ($request->getGet("term")) {
        ...
        $filter->likeAll    = $request->getGet("term");
        $list               = $comboService->loadLocalidades($filter);
        ...
        $filter         = new LocalidadFilter($request->getGet("id"));
        $obj            = $comboService->loadLocalidades($filter)[0];
        $row_array['id']    = $obj->idLocalidad;
        $row_array['text']  = $obj->localidad." - ".$obj->partido;
        array_push($return_arr,$row_array);
