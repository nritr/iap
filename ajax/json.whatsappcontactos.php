...
$whatService        = new WhatsappContactoService();
...
if(($request->getGet("term") && strlen($request->getGet("term")) > 0) || ($request->getGet("id") && is_numeric($request->getGet("id")))) {
    ...
    if ($request->getGet("term")) {
        $filter             = new WhatsappContactoFilter();
        $filter->nombreLike = $request->getGet("term");
        $list               = $whatService->load($filter);
        foreach ($list as $value) {
            
            $row_array['id']    = $value->idWhatsappContacto;
            $row_array['text']  = $value->nombre;
            array_push($return_arr,$row_array);
            
        }
    } elseif ($request->getGet("id")) {
        $obj                = $whatService->getById($request->getGet("id"));
        $row_array['id']    = $obj->idWhatsappContacto;
    ...
    $list = $comboService->loadLocalidades();
    foreach ($list as $value) {
        
        $row_array['id']    = $value->idLocalidad;
        $row_array['text']  = $value->localidad." - ".$value->partido;
        array_push($return_arr,$row_array);
