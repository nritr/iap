...
function addImg(data) {
	//path,name,id,empleado
	path 	= json.data.file; 
	name	= json.data.name;
	id		= json.data.id;
	proyecto= json.data.proyecto;

	strOptions = getOptions();
	if (path) {
...
function getOptions() {
	...
	for (i=0;i<json.length;i++) {
		ret += "<option value='"+json[i].idDato+"' "+(json[i].idDato==85 ? 'selected' : '')+">"+json[i].nombre;
	}
	return ret;
}
...
function attachElements() {
	...
	$("[data-attach-periodo]").change(function () {
		if ($(this).val().length>4) {
			objFocus = this;
			var id = $(this).attr('data-attach-periodo');
			var data = new Object();
			data.tipo 			= "change-periodo";
			data.id_documento	= id;
			data.value			= $(this).val();
			loadData('/admin/modulos/documentacion/json/json.documentos.php',fnSuccessFormUpdateResponse,data,fnErrorFormResponse);
		} else {
