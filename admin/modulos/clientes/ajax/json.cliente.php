...
if ($data['tipo']=='updatecreate') {
	...
	$obj->nombre		= $nombre;
	//$obj->sucursal		= new Sucursal(SUCURSAL_DEFAULT); //obj
	$obj->email			= $email;
...
} else if ($data['tipo']=='updatecreate_planta') {
    $id_planta      = ($data['id_planta']!="" ? $data['id_planta'] : null);
    ...
    //$planta                 = new Planta($id_planta, $nombre, $id_cliente,$direccion);
    $planta->observacion    = $observacion;
    $planta->contacto       = $contacto;
    $planta->email          = $email;
    $planta->telefono       = $telefono;
    $planta->celular        = $celular;
    $planta->estado         = 1;
    $obj = $plantaService->saveUpdate($planta);
    $errorManager->setData($obj);
} else if ($data['tipo']=='list-plamntas-by-client') {
    $id_cliente = $data['id_cliente'];
    //$filter             = new PlantaFilter();
    $filter->idCliente  = $id_cliente;
    $list               = $plantaService->load($filter);
